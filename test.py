...
### tflms_v2.py ###
'''
def rotate_image(images, masks, degree=180):
    """image : Batch_size, 101, 101, 1)"""
    images = np.squeeze(images)
    masks = np.squeeze(masks)
    angles = [np.random.uniform(-degree, degree) for _ in images]
    rotated_images = np.array([rotate(image, angle, mode='edge') for angle, image in zip(angles, images)]).reshape(-1, 101, 101, 1)
    rotated_masks = np.array([rotate(mask, angle, mode='edge') for angle, mask in zip(angles, masks)]).reshape(-1, 101, 101, 1)
...
xtf.keras.backend.set_session(sess)
'''

densepath = "dense/lovasz.h5"
index = 4

_, x_valid, _, y_valid = read_dataset(n_splits=5, index=index, shuffle=True, seed=1234567890)

model = load_model(densepath, custom_objects={'Bilinear':Bilinear, 'focal_loss':focal_loss, 'lovasz_loss':lovasz_loss, 'GlorotUniform':tf.glorot_uniform_initializer
})

model.summary()
preds_valid = model.predict(x_valid)

ious = iou_metric_batch(y_valid, preds_valid)
print(ious)



###############################################################################
for x_true, y_true, y_pred in zip(x_valid, y_valid, preds_valid):
    ...
    if ious == 0:
        plt.imshow(np.hstack((np.squeeze(x_true), np.squeeze(y_true), np.squeeze(y_pred))))
        plt.show()
